...
- 支持为每个任务选择一个或多个通知渠道；内置邮件，并可配置 Bark、Gotify、PushDeer、PushPlus、Telegram、钉钉、企业微信、飞书和自定义 Webhook。
- 支持完成确认邮件，让收件人知道系统已经收到回复。
- 支持普通用户注册和登录，任务数量不设上限。
...
升级到通知渠道版本后也需要运行这条迁移命令。迁移会保留现有任务，并把它们的通知渠道默认设置为“邮件”。
...
- 注册：普通用户可以直接用邮箱和密码注册，也可以使用 Linux.do OAuth 登录，任务数量不设上限。
- 管理员：使用 `ADMIN_TOKEN` 进入管理台，可查看和管理全部任务、用户、注册开关、邀请码、公告和最近 30 天日志，并手动触发到期检查。

管理员可以在“设置”里关闭注册、开启一次性邀请码、批量生成邀请码、设置邀请码过期时间、维护公告。管理员也可以在“设置 → 通知渠道”中新增、编辑、停用和删除推送渠道；普通用户只能看到已启用渠道，不能读取渠道凭据。停用或删除渠道时，引用它且没有其他渠道的任务会自动回退到邮件。邀请码一人一码，使用后不能重复使用，过期后也不能使用。封禁用户后，该用户不能登录，现有提醒任务会全部失效；删除用户会清理该用户的任务、提醒轮次、发送记录和用户侧日志。

创建提醒时需要填写：

- 任务类型：普通定时通知按通知间隔持续发送；确认完成任务在未完成时发送追提醒，并且必须选择邮件渠道。
- 收件邮箱：仅在选择邮件渠道时显示并必填。
- 标题：邮件标题，也是任务名称。
- 正文：提醒邮件正文。
...
- 追提醒间隔：仅确认完成任务使用，表示本轮未确认完成时多久再次发送。
